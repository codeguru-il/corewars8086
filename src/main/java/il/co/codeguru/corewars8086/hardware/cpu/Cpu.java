...
import il.co.codeguru.corewars8086.hardware.*;
...
public class Cpu extends AbstractCpu {
	 ...
	 */
	public synchronized void nextOpcode() throws CpuException, MemoryException, InterruptException {
		...
		} catch(CpuException | MemoryException e) {
			gotoPrevState();
			throw e;
		} finally {
			instructionCounter++;
	...
	protected void processOpcode(byte opcode) throws CpuException, MemoryException, InterruptException {
		switch(opcode & 0xF0) {
			case 0x00:
				opcode0X(opcode);
				break;
	 ...
	 */
	public void interrupt( int intnum ) throws IntOpcodeException, MemoryException {
        if (intnum == (byte)0x86) {
            int86();
        } else if (intnum == (byte)0x87) {
            int87();
        } else {
	...
	protected void out(int port, int data) {
		if( ioHandler != null ) {
	...
	protected void opcode2X(byte opcode) throws CpuException, MemoryException, InterruptException {
		switch (opcode) {
	...
	protected void opcode3X(byte opcode) throws CpuException, MemoryException, InterruptException {
		switch (opcode) {
	...
	protected void opcode7X(byte opcode) throws MemoryException {
		...
		if (branch) {
			m_state.setIP(m_state.getIP() + offset);
		}
	}
	...
	protected void opcode8X(byte opcode) throws CpuException, MemoryException {
		switch (opcode) {
			...
			case (byte)0x87: // XCHG reg16, [X]
				m_indirect.reset();
				int tmpWord = m_indirect.getReg16();
			...
			case (byte)0x8D: // LEA reg16, [X]
				m_indirect.reset();
				AbstractAddress address = m_indirect.getMemAddress();
				if (address == null) {
	...
	protected void opcodeCX(byte opcode) throws CpuException, MemoryException, InterruptException {
		short sizeToPop;
		AbstractAddress address1 = null;
		AbstractAddress address2 = null;
		switch (opcode) {
			case (byte)0xC0:
			case (byte)0xC1:
			...
			case (byte)0xC2: // RETN [imm16]
				...
				m_state.setIP(pop());
				m_state.setSP((m_state.getSP() + sizeToPop));
				break;
			case (byte)0xC3: // RETN
				m_state.setIP(pop());
				break;
			case (byte)0xC4: // LES reg16, [X]
				...
				address2 = m_machine.newAddress(
						address1.getSegment(), (address1.getOffset() + 2));
	...
	protected void opcodeDX(byte opcode) throws CpuException, MemoryException {
		switch (opcode) {
			...
			case (byte)0xD7: // XLAT, XLATB
				AbstractAddress address = m_machine.newAddress(m_state.getDS(),
						(m_state.getBX() + Unsigned.unsignedByte(m_state.getAL())));
				m_state.setAL(m_memory.readByte(address));
				break;
	...
	protected void opcodeEX(byte opcode) throws CpuException, MemoryException {
		...
		switch (opcode) {
			case (byte)0xE0: // LOOPNZ, LOOPNE
				...
				newCX = (m_state.getCX() - 1);
				m_state.setCX(newCX);
				if ((newCX != 0) && (!m_state.getZeroFlag())) {
					m_state.setIP((m_state.getIP() + offset8));
				}
				break;
			...
			case (byte)0xE7: // OUT imm8, AX
				out(m_fetcher.nextByte(), m_state.getAX());
				break;
			case (byte)0xE8: // CALL near imm16
				offset16 = m_fetcher.nextWord();
				callNear((m_state.getIP() + offset16));
				break;
			case (byte)0xE9: // JMP near imm16
				offset16 = m_fetcher.nextWord();
				m_state.setIP((m_state.getIP() + offset16));
				break;
			...
			case (byte)0xEF: // OUT DX, AX
				out(m_state.getDX(), m_state.getAX());
				break;
			default:
				throw new RuntimeException();
		}
			...
			case (byte)0xF7: // <?> word ptr [X]
				...
				switch (m_indirect.getRegIndex()) {
					...
					case 1:
						throw new InvalidOpcodeException();
					case 2: // NOT
						m_indirect.setMem16((m_indirect.getMem16() ^ 0xFFFF));
						break;
					case 3: // NEG
						m_indirect.setMem16(sub16(0, m_indirect.getMem16()));
						break;
					case 4: {// MUL
						// multiply
						int result =
						...
						m_state.setDX((result >> 16));
						m_state.setAX(result);
					...
					case 6: {// DIV
						...
						m_state.setAX(quotient);
						m_state.setDX((int)(tmp % divisor));
						m_state.setUndefinedFlags((
								CpuState.FLAGS_MASK_OVERFLOW|
										CpuState.FLAGS_MASK_SIGN|
										CpuState.FLAGS_MASK_ZERO|
	...
	protected void push(int value) throws MemoryException {
		m_state.setSP(m_state.getSP() - 2);
		AbstractAddress stackPtr = m_machine.newAddress(
	...
	protected void updateFlags16(int value) {
		...
		// TODO: update overflow flag
		updateFlagsNoCarryOverflow16(value);
	}
	 ...
	 */
	protected void updateFlagsNoCarryOverflow16(int val) {
	...
	protected byte add8(byte value1, byte value2) {
		int result16 =
				(Unsigned.unsignedByte(value1) + Unsigned.unsignedByte(value2));
	 ...
	 * Note: does not modify the carry flag.
	 */
	protected int inc16(int value) {
		boolean oldCarry = m_state.getCarryFlag();
		int result = add16(value, 1);
		m_state.setCarryFlag(oldCarry);
		return result;
	}
	 ...
	 * Note: does not modify the carry flag.
	 */
	protected int dec16(int value) {
		boolean oldCarry = m_state.getCarryFlag();
		int result = sub16(value, 1);
		m_state.setCarryFlag(oldCarry);
		return result;
	}
	 ...
	 */
	protected int or16(int value1, int value2) {
		int result32 =
				Unsigned.unsignedShort(value1) | Unsigned.unsignedShort(value2);
		int result16 = result32;
	...
	protected byte adc8(byte value1, byte value2) {
		...
		int result16 =
				(Unsigned.unsignedByte(value1) + Unsigned.unsignedByte(value2)+carry);
		updateFlags8(result16);
		result16 = (value1+value2+carry);
	 ...
	 * the result.
	 */
	protected int adc16(int value1, int value2) {
		int carry = m_state.getCarryFlag()?1:0;
		int result32 = carry +
				Unsigned.unsignedShort(value1) + Unsigned.unsignedShort(value2);
		updateFlags16(result32);
		result32=value1+value2+carry;
		m_state.setOverflowFlag(result32>0x7fff||result32<-0x8000);
		m_state.setAuxFlag((value1&0xf)+(value2&0xf)+carry>0xf);
		return result32;
	}
	...
	protected byte sbb8(byte value1, byte value2) {
		...
		updateFlags8((Unsigned.unsignedByte(value1)- Unsigned.unsignedByte(value2))-borrow);
		int result16=((byte)value1-(byte)value2-borrow);
		m_state.setOverflowFlag(result16 > 0x7f || result16 < -0x80);
	 ...
	 */
	protected int and16(int value1, int value2) {
		int result32 =
				Unsigned.unsignedShort(value1) & Unsigned.unsignedShort(value2);
		int result16 = result32;
		updateFlags16(result32);
	...
	protected byte sub8(byte value1, byte value2) {
		updateFlags8((Unsigned.unsignedByte(value1)- Unsigned.unsignedByte(value2)));
		int result16=((byte)value1-(byte)value2);
		m_state.setOverflowFlag(result16 > 0x7f || result16 < -0x80);
	 ...
	 */
	protected int sub16(int value1, int value2) {
		updateFlags16(Unsigned.unsignedShort(value1) - Unsigned.unsignedShort(value2));
		int result32=value1-value2;
		m_state.setOverflowFlag(result32 > 0x7fff || result32 < -0x8000);
		m_state.setAuxFlag((value1&0xf)<(value2&0xf));
	 ...
	 */
	protected int xor16(int value1, int value2) {
		int result32 =
				Unsigned.unsignedShort(value1) ^ Unsigned.unsignedShort(value2);
		int result16 = result32;
		updateFlags16(result32);
	 ...
	 */
	protected void callNear(int offset) throws MemoryException {
		AbstractAddress from = m_machine.newAddress(prevState.getCS(), prevState.getIP());
		AbstractAddress to = m_machine.newAddress(m_state.getCS(), offset);
	...
	protected void movsb() throws MemoryException {
		...
		m_state.setSI((m_state.getSI() + diff));
		m_state.setDI((m_state.getDI() + diff));
	}
	...
	protected void cmpsb() throws MemoryException {
		AbstractAddress address1 =
				m_machine.newAddress(m_state.getDS(), m_state.getSI());
		AbstractAddress address2 =
				m_machine.newAddress(m_state.getES(), m_state.getDI());
	...
	protected void stosdw() throws MemoryException {
		...
		AbstractAddress address2 =
				m_machine.newAddress(m_state.getES(), (m_state.getDI() + 2));
    ...
    private void int86() throws MemoryException {
        int bombCount = m_state.getBomb1Count();
        if (bombCount != 0) {
    ...
    private void int87() throws MemoryException {
        int bombCount = m_state.getBomb2Count();
        if (bombCount != 0) {
            m_state.setBomb2Count(bombCount - 1);

            for (int i = 0; i <= 0xFFFF; ++i) {
                int diff = (m_state.getDirectionFlag() ? -i : i);

                Address address1 = new Address(
                    m_state.getES(), (m_state.getDI() + diff));

                if (m_memory.readWord(address1) == m_state.getAX()) {
                    Address address2 = new Address(
                        m_state.getES(), (m_state.getDI() + diff + 2));
