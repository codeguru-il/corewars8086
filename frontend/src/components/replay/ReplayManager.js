// frontend/src/components/replay/ReplayManager.js
    ...
    _parse(fileContent) {
        ...
        lines.forEach((line, index) => {
            try {
                ...
                if (event.type === "WAR_START") { this.warStartData = payload; }
                if (cycle !== undefined) {
                ...
                if (event.type === 'WAR_END') { this.totalCycles = cycle; }
            } catch (e) {
        ...
        if (this.totalCycles === 0) { this.totalCycles = this.warStartData.max_rounds || 200000; }
    }

    getWarStartData() { return this.warStartData; }
    getEventsInCycle(cycle) { return this.eventsByCycle.get(cycle) || []; }
    getTotalCycles() { return this.totalCycles; }
}